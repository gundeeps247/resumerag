...
**Short:** I moved generation to where the compute is — the visitor's own machine. The deployed site is static files plus two small functions, and by default the language model runs in the browser tab: LFM2 1.2B as 4-bit ONNX through Transformers.js, on WebGPU where available and on WebAssembly otherwise, about 850 MB downloaded once from the Hugging Face CDN and then served from the browser cache. So the public demo generates real cited answers with no API key, no account and no server model. Two other modes exist: private mode, where the browser calls the visitor's own Ollama (they allow the site's origin with `OLLAMA_ORIGINS`), and a hosted open-weight model behind the proxy, protected by an access code, a model allowlist and a rate limit. The connection defaults to "automatic": use the server's model when it has a reachable one, otherwise the browser's.

### 10a. How did you choose the in-browser model?

**Short:** I benchmarked it instead of guessing. `scripts/llm-bench` builds prompts with the app's own pipeline — real retrieval, reranking, context building and system prompts — then runs each candidate in headless Chrome over WebGPU and scores the replies with the app's own citation, number and schema checks: six grounded questions, two unanswerable ones and two structured-JSON workflows. LFM2 1.2B won: 92% of required facts, a valid citation in every answer, zero invented numbers, and both unanswerable questions refused. Qwen2.5 0.5B failed all three properties; Qwen3 0.6B stayed honest but missed a third of the facts and was the slowest; gemma-3 1B hung the GPU device and Qwen2.5 1.5B (1.8 GB) could not even create a session on integrated graphics, which disqualifies them as a default for strangers' laptops.

**Two findings worth telling:** first, the smaller `q4f16` exports — 4-bit weights with 16-bit activations — produced repeated tokens and random Chinese on the benchmark GPU, while the same weights as `q4` (32-bit activations) matched the CPU output exactly; I ship only `q4`. Second, structured output is the real weakness in the browser, because Transformers.js has no grammar-constrained decoding like Ollama's JSON schema mode, so those workflows lean on the repair retry and the deterministic fallbacks.

### 11. What happens when no LLM is available?

**Short:** Graceful degradation, and it is now the last resort rather than the normal deployed state, because the in-browser model covers that case. If even that cannot run — an old browser, or a device without the memory for it — retrieval still runs and the Ask page shows the most relevant sentences from the top passages as quotes with citations, clearly labelled. Workflows fall back to deterministic results: template interview questions from flagged claims, rule-based JD summaries, heuristic mock-interview scoring. Nothing is generated, so nothing can be hallucinated. If a model is connected but stops responding, a stall timeout (240 s to first output, 90 s between chunks) aborts the request and the same fallbacks apply, so the UI never spins forever.
